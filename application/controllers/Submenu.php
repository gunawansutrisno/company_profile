...
class Submenu extends MY_Controller {
    ...
    var $meta_title = "MOLINDO INCORPORATED | SubMenu Utama";
    var $meta_desc = "MOLINDO INCORPORATED";
    var $main_title = "SubMenu Utama";
    var $base_url = "";
    var $upload_dir = "";
    var $upload_url = "";
    var $limit = "10";
    ...
    public function index() {
        ...
        $dt = array(
            ...
            "custom_js" => array(
                ASSETS_JS_URL . "form/jenisbarang.js",
    ...
    private function _home() {
        ...
        $data = $this->jenisbarang_model->getDataIndex($start, $this->limit, $search);
        $countTotal = $this->jenisbarang_model->getCountDataIndex($search);
    ...
    public function ajax_list() {
        ...
        $list = $this->jenisbarang_model->get_datatables($nama);
//        echoPre($this->db->last_query());
        $data = array();
        foreach ($list as $dt) {
            $row = array();
            $row[] = $dt->name;
            $row[] = '<a href="#" onclick="loadData(' . "'" . $this->base_url . 'edit/' . $dt->id . "'" . ')">' . $dt->submenu . '</a>';
            if($dt->status == 0){
...
//            "recordsFiltered" => $this->jenisbarang_model->count_filtered($nama),
            "data" => $data,
        );
    ...
    public function edit($term_id="") {
        ...
        $data = $this->jenisbarang_model->getDetail($term_id);
        $resData = $data[0];
    ...
    private function _saveData($id = '') {
        ...
        if (!empty($code)) {
            if (!empty($id)) {
                $edit = array(
                    "submenu_url" => url_title($code),
                   "menu_id" => $menu_id,
                    "submenu" => $code,
                );
                $res = $this->jenisbarang_model->updateDetail($edit, $id);
                if ($res['status'] == true) {
                    $return = array(
                        "status" => "success",
                        "message" => "Success to update Data Sub Menu $code."
                    );
                }
            }
            // insert category
...
//                echoPre($insert);exit;
                $res = $this->jenisbarang_model->saveData($insert);
                if ($res['status'] == true) {
                    $return = array(
                        "status" => "success",
 ...
 public function delete($id) {
        $del_author = $this->jenisbarang_model->delete($id);
        $del_author['status'];
    ...
    public function active($id) {
        ...
        $res = $this->jenisbarang_model->actived($id);
        if ($res['status'] == true) {
            $alert = array(
                "status" => "success",
    ...
    public function deactive($id) {
        ...
        $res = $this->jenisbarang_model->deactived($id);
        if ($res['status'] == true) {
            $alert = array(
                "status" => "success",
